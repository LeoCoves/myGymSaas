...
const GymPaymentPlansPage = () => {
  ...
  const handleDelete = async () => {
    try {
      ...
      setPaymentPlans(paymentPlans.filter(plan => plan.idGymCustomPaymentPlan !== selectedId));
    } catch (error) {
  ...
  return (
    <div className="flex-1 ml-30 p-6">
      ...
      <div className="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3">
        ...
        >
          <div className="flex-1">
            <h3 id={`tier-${plan.idGymCustomPaymentPlan}`} className="text-base font-semibold text-red-600">
              {plan.name}
            </h3>
              ...
              {plan.features && plan.features.length > 0 ? (
                plan.features.map((feature, i) => (
                  <li key={i} className="flex gap-x-3">
                    <svg className="h-6 w-5 text-orange-600" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"
                    >
